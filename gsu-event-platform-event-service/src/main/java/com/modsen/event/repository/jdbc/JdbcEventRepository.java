...
public class JdbcEventRepository implements EventRepository {
    ...
    public Optional<Event> findById(UUID uuid) {
        final String sql = """
                SELECT
                    ...
                    jsonb_build_object(
                        'id', event_statuses.id,
                        'name', event_statuses.name
                    ) AS event_status,
                ...
                INNER JOIN event_statuses ON events.status_id = event_statuses.id
                ...
                WHERE events.id = ?
                GROUP BY events.id, event_types.id, event_statuses.id
                """;
    ...
    public List<Event> findByStatus(EventStatus status, UUID lastFetchedId, int size) {
        final String sql = """
                ...
                LEFT JOIN tags ON event_tags.tag_id = tags.id
                LEFT JOIN event_attachments ON events.id = event_attachments.event_id
                WHERE events.id > ? AND event_statuses.id = ?
                GROUP BY events.id, event_types.id, event_statuses.id
                ORDER BY events.id
                LIMIT ?
                """;
    ...
    public List<Event> findBetweenDates(LocalDateTime start, LocalDateTime end, UUID lastFetchedId, int size) {
        final String sql = """
                ...
                LEFT JOIN tags ON event_tags.tag_id = tags.id
                LEFT JOIN event_attachments ON events.id = event_attachments.event_id
                WHERE events.id > ? AND events.start_time BETWEEN ? AND ?
                GROUP BY events.id, event_types.id, event_statuses.id
                ORDER BY events.id
                LIMIT ?
                """;
    ...
    public List<Event> findByRegistrationDeadlineApproaching(LocalDateTime deadline, UUID lastFetchedId, int size) {
        final String sql = """
                ...
                GROUP BY events.id, events.registration_deadline, event_types.id, event_statuses.id
    ...
    public <S extends Event> S save(S entity) {
        final String sql = """
                INSERT INTO events (
                    ...
                    eligibility_criteria, budget, image_url, registration_deadline, type_id, status_id
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """;
        ...
        jdbcClient.sql(sql)
                ...
                .param(17, entity.type().id())
                .param(18, entity.status().id())
                .update();
    ...
    public <S extends Event> S update(UUID id, S entity) {
        ...
        int updatedRows = jdbcClient.sql(updateEventSql)
                ...
                .param(14, entity.type().id())
                .param(15, entity.status().id())
                .param(16, id)
                .update();
