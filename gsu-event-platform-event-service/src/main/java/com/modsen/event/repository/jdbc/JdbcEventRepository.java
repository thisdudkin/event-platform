...
public class JdbcEventRepository implements EventRepository {
    ...
    @Override
    public Iterable<Event> searchEvents(String query, UUID lastFetchedId, int size) {
        final String sql = """
                SELECT
                    events.id AS event_id,
                    events.title,
                ...
                AND (
                    events.title ILIKE '%' || ? || '%'
                    OR events.description ILIKE '%' || ? || '%'
                    OR events.location ILIKE '%' || ? || '%'
                    OR tags.name ILIKE '%' || ? || '%'
                    OR events.contact_info ILIKE '%' || ? || '%'
                )
                GROUP BY events.id, events.type, events.status
                ORDER BY events.id
                LIMIT ?
